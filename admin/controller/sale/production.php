<?php
class ControllerSaleProduction extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('sale/production');
	...
	public function delete() {
		...
		$this->response->redirect($this->url->link('sale/production', 'user_token=' . $this->session->data['user_token'] . $url, true));
	}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/production', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		$data['invoice'] = $this->url->link('sale/production/invoice', 'user_token=' . $this->session->data['user_token'], true);
		$data['shipping'] = $this->url->link('sale/production/shipping', 'user_token=' . $this->session->data['user_token'], true);
		$data['add'] = $this->url->link('sale/production/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = str_replace('&amp;', '&', $this->url->link('sale/production/delete', 'user_token=' . $this->session->data['user_token'] . $url, true));
	...
	public function getForm() {
		...
		$data['cancel'] = $this->url->link('sale/production', 'user_token=' . $this->session->data['user_token'] . $url, true);

		$data['user_token'] = $this->session->data['user_token'];

		if (isset($this->request->get['order_id'])) {
			$order_info = $this->model_sale_production->getOrder($this->request->get['order_id']);
		}
		...
		if (!empty($order_info)) {
			...
			$products = $this->model_sale_production->getOrderProducts($this->request->get['order_id']);
			...
			foreach ($products as $product) {
				$data['order_products'][] = array(
					...
					'name'       => $product['name'],
					'model'      => $product['model'],
					'option'     => $this->model_sale_production->getOrderOptions($this->request->get['order_id'], $product['order_product_id']),
					'quantity'   => $product['quantity'],
					'price'      => $product['price'],
					'total'      => $product['total'],
			...
			// Vouchers
			$data['order_vouchers'] = $this->model_sale_production->getOrderVouchers($this->request->get['order_id']);
			...
			$order_totals = $this->model_sale_production->getOrderTotals($this->request->get['order_id']);
		...
		$this->response->setOutput($this->load->view('sale/production_form', $data));
	}
	...
	public function info() {
		...
		$order_info = $this->model_sale_production->getOrder($order_id);
		...
		if ($order_info) {
			...
			$data['shipping'] = $this->url->link('sale/production/shipping', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . (int)$this->request->get['order_id'], true);
			$data['invoice'] = $this->url->link('sale/production/invoice', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . (int)$this->request->get['order_id'], true);
			$data['edit'] = $this->url->link('sale/production/edit', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . (int)$this->request->get['order_id'], true);
			$data['cancel'] = $this->url->link('sale/production', 'user_token=' . $this->session->data['user_token'] . $url, true);
			...
			/*if($order_info['customer_group_id']==4){
				...
				$opt = $this->model_sale_production->getOrderOption($order_option_id);
				$orderid = $opt[0]['order_id'];
				$order_product_id = $opt[0]['order_product_id'];
			}*/
			
			$this->load->model('tool/image');
            $products = $this->model_sale_production->getOrderProducts($orderid);
			$subtotal=0;
			$tax=0;
			$shipping=0;
			$total=0;
			...
			$vouchers = $this->model_sale_production->getOrderVouchers($this->request->get['order_id']);
		...
		/*	$totals = $this->model_sale_production->getOrderTotals($this->request->get['order_id']);
			...
			$this->response->setOutput($this->load->view('sale/production_info', $data));
		} else {
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'sale/production')) {
	...
	public function createInvoiceNo() {
		...
		if (!$this->user->hasPermission('modify', 'sale/production')) {
			...
			$invoice_no = $this->model_sale_production->createInvoiceNo($order_id);
	...
	public function history() {
		...
		$results = $this->model_sale_production->getOrderHistories($this->request->get['order_id'], ($page - 1) * 10, 10);

		foreach ($results as $result) {
			$data['histories'][] = array(
				'notify'     => $result['notify'] ? $this->language->get('text_yes') : $this->language->get('text_no'),
				'status'     => $result['status'],
				'shopping_method'     => isset($result['shopping_method'])?( $result['shopping_method'].' :  '.$result['shopping_number']):'',
				'comment'    => nl2br($result['comment']),
				'date_added' => date($this->language->get('date_format_short'), strtotime($result['date_added']))
			);
		}

		$history_total = $this->model_sale_production->getTotalOrderHistories($this->request->get['order_id']);

		$pagination = new Pagination();
		$pagination->total = $history_total;
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('sale/production/history', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $this->request->get['order_id'] . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($history_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($history_total - 10)) ? $history_total : ((($page - 1) * 10) + 10), $history_total, ceil($history_total / 10));

		$this->response->setOutput($this->load->view('sale/production_history', $data));
	}
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$products = $this->model_sale_production->getOrderProducts($order_id);

				foreach ($products as $product) {
					$option_data = array();

					$options = $this->model_sale_production->getOrderOptions($order_id, $product['order_product_id']);
				...
				$vouchers = $this->model_sale_production->getOrderVouchers($order_id);
				...
				$totals = $this->model_sale_production->getOrderTotals($order_id);
		...
		$this->response->setOutput($this->load->view('sale/production_invoice', $data));
	}
...
//print_r($order_info['shipping_country']);
		...
		$this->response->setOutput($this->load->view('sale/production_shipping', $data));
	}
}
