...
class ControllerCatalogProduct extends Controller {
	...
	public function loadList() {
		...
		foreach ($results as $result) {
			...
			}else if(strpos($result['image'],"http")!== false){
				$image = $result['image'];
			} else {
		...
		$this->response->setOutput($this->load->view('catalog/product_item', $data));
	}
	...
	protected function getForm() {
		...
		} else if(strpos($product_info['image'],"http")!== false){
		...
		foreach ($product_images as $product_image) {
			...
			}else if(strpos($product_image['image'],"http")!== false){
				$image = $product_image['image'];
				$thumb = $product_image['image'];
			} else {
				$image = '';
			...
			$data['product_images'][] = array(
				'image'      => $image,
				'thumb'      => strpos($thumb,"http")!== false?$thumb:$this->model_tool_image->resize($thumb, 100, 100),
