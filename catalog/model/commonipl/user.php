<?php
class ModelCommoniplUser extends Model {
	public function updateUser($customer_id,$data) {
		
		$user_info = $this->getUser($customer_id);
		
		if(isset($user_info)&&count($user_info)>0){
				$this->db->query(" UPDATE " . DB_PREFIX . "shopify_user SET store_id = '" . (int)$this->config->get('config_store_id') . "', language_id = '" . (int)$this->config->get('config_language_id') . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "' WHERE customer_id = '" . (int)$customer_id . "'");
		
		}else{
				$this->db->query("INSERT INTO " . DB_PREFIX . "shopify_user SET customer_id = '" . (int)$customer_id. "', store_id = '" . (int)$this->config->get('config_store_id') . "', language_id = '" . (int)$this->config->get('config_language_id') . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', date_added = NOW()");
		}
	...
	public function getUser($customer_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "shopify_user WHERE customer_id = '" . (int)$customer_id . "'");
		return $query->row;
	}
	...
	public function getTotalUsersByEmail($customer_id,$email) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "shopify_user WHERE LOWER(email) = '" . $this->db->escape(utf8_strtolower($email)) . "' AND  customer_id<>'".(int)$customer_id."'");
